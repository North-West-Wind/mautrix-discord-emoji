...
// this is a modified copy of maunium.net/go/mautrix/format/htmlparser.go
// it is extended to handle img tags
...
type EmoticonConverter func(src, alt string, ctx Context) string
...
// ExtendedHTMLParser is a somewhat customizable Matrix HTML parser.
type ExtendedHTMLParser struct {
	...
	EmoticonConverter       EmoticonConverter
}
...
func (parser *ExtendedHTMLParser) maybeGetAttribute(node *html.Node, attribute string) (string, bool) {
...
func (parser *ExtendedHTMLParser) getAttribute(node *html.Node, attribute string) string {
...
func (parser *ExtendedHTMLParser) listToString(node *html.Node, ctx Context) string {
...
func (parser *ExtendedHTMLParser) basicFormatToString(node *html.Node, ctx Context) string {
...
func (parser *ExtendedHTMLParser) spanToString(node *html.Node, ctx Context) string {
	str := parser.nodeToTagAwareString(node.FirstChild, ctx)
	if node.Data == "span" {
...
func (parser *ExtendedHTMLParser) headerToString(node *html.Node, ctx Context) string {
...
func (parser *ExtendedHTMLParser) blockquoteToString(node *html.Node, ctx Context) string {
	str := parser.nodeToTagAwareString(node.FirstChild, ctx)
...
func (parser *ExtendedHTMLParser) linkToString(node *html.Node, ctx Context) string {
	str := parser.nodeToTagAwareString(node.FirstChild, ctx)
...
func (parser *ExtendedHTMLParser) tagToString(node *html.Node, ctx Context) string {
	...
	case "img":
		_, isEmoticon := parser.maybeGetAttribute(node, "data-mx-emoticon")
		if isEmoticon {
			src, hasSrc := parser.maybeGetAttribute(node, "src")
			alt, hasAlt := parser.maybeGetAttribute(node, "alt")
			if !hasSrc {
				if !hasAlt {
					return ""
				}
			...
			if parser.EmoticonConverter != nil {
				return parser.EmoticonConverter(src, alt, ctx)
			}
			if hasAlt {
				return fmt.Sprintf(":%s:", alt)
			}
...
func (parser *ExtendedHTMLParser) singleNodeToString(node *html.Node, ctx Context) TaggedString {
	switch node.Type {
	case html.TextNode:
...
func (parser *ExtendedHTMLParser) nodeToTaggedStrings(node *html.Node, ctx Context) (strs []TaggedString) {
...
func (parser *ExtendedHTMLParser) isBlockTag(tag string) bool {
...
func (parser *ExtendedHTMLParser) nodeToTagAwareString(node *html.Node, ctx Context) string {
...
func (parser *ExtendedHTMLParser) nodeToStrings(node *html.Node, ctx Context) (strs []string) {
	for ; node != nil; node = node.NextSibling {
...
func (parser *ExtendedHTMLParser) nodeToString(node *html.Node, ctx Context) string {
	return strings.Join(parser.nodeToStrings(node, ctx), "")
}

// Parse converts Matrix HTML into text using the settings in this parser.
func (parser *ExtendedHTMLParser) Parse(htmlData string, ctx Context) string {
